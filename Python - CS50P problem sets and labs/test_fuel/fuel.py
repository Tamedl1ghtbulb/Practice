def main():
    while True:
        try:
            l= input('Fraction: ')
            try:
                t = l.index("/") #nacin za unos / buduci da dole vec imamo ispistan exception za ValueError
            except:
                continue
            a = l.split("/")
            x = a[0]
...
#                raise ValueError
        ...
        break
def convert(a):
    l = a.index("/") #najlaksi nacin ya unos bilo cega osim / buduci da dole vec imamo ispistan exeption za ValueError
    a = a.split("/")
    x = a[0]
    y = a[1]
    if x.find(".")!= -1 or x.find(",")!=-1:
